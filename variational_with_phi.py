...
def update_phi_s(phi_s1,phi_s2,user_index,author_index,tweet_index,U_SHP,U_RTE,V_SHP,V_RTE,Y_SHP,Y_RTE,THETA_SHP,THETA_RTE):
...
def update_phi_z(phi_z,word_index,tweet_index,THETA_SHP,THETA_RTE,BETA_SHP,BETA_RTE):
...
def  ELBO(hp,X_SHP,X_RTE,U_SHP,U_RTE,Y_SHP,Y_RTE,V_SHP,V_RTE,THETA_SHP,THETA_RTE,BETA_SHP,BETA_RTE,length,traindata,tweet,tweetdata,phi_s1,phi_s2,phi_z):
    ...
    X_expectation=make_nozero(X_SHP)/make_nozero(X_RTE)
    #make_nozero(X_expectation)
    term1=np.sum((hp+hp*topic-X_SHP)*(digamma(X_SHP)-np.log(X_RTE))-(hp-X_RTE)*X_expectation+gammaln(X_SHP)-X_SHP*np.log(X_RTE))
    U_expectation=make_nozero(U_SHP)/make_nozero(U_RTE)
    #make_nozero(U_expectation)
    ...
    for i in range(tweet):
        for j in tweetdata[str(i)]:
            ...
            #z=THETA_expectation[i,:]*BETA_expectation[int(j),:]
            #make_nozero(z)
            temp_z=np.zeros((1,topic),dtype=np.float64)
...
def savemodel(X_SHP,X_RTE,U_SHP,U_RTE,Y_SHP,Y_RTE,V_SHP,V_RTE,THETA_SHP,THETA_RTE,BETA_SHP,BETA_RTE):
    np.savetxt('model/X_SHP.txt',X_SHP)
    np.savetxt('model/X_RTE.txt',X_RTE)
    np.savetxt('model/U_SHP.txt',U_SHP)
    np.savetxt('model/U_RTE.txt',U_RTE)
    np.savetxt('model/Y_SHP.txt',Y_SHP)
    np.savetxt('model/Y_RTE.txt',Y_RTE)
    np.savetxt('model/V_SHP.txt',V_SHP)
    np.savetxt('model/V_RTE.txt',V_RTE)      
    np.savetxt('model/THETA_SHP.txt',THETA_SHP)
    np.savetxt('model/THETA_RTE.txt',THETA_RTE)
    np.savetxt('model/BETA_SHP.txt',BETA_SHP)
    np.savetxt('model/BETA_RTE.txt',BETA_RTE)   
...
def process(user,tweet,word,author,topic,hp,shp_off,rte_off,BTM,length):
    ...
    iter=0
    while 1:
        #first ,we should calculate the multinomial parameter
        for i in range(length):
            phi_s1,phi_s2 = update_phi_s(phi_s1,phi_s2,traindata[i,0],traindata[i,1],traindata[i,2],U_SHP,U_RTE,V_SHP,V_RTE,Y_SHP,Y_RTE,THETA_SHP,THETA_RTE)
            #break
        #print np.sum(phi_s1)+np.sum(phi_s2)
        #break
        for i in range(tweet):
            for j in tweetdata[str(i)]:
                phi_z = update_phi_z(phi_z,int(j),i,THETA_SHP,THETA_RTE,BETA_SHP,BETA_RTE)
        elbo=ELBO(hp,X_SHP,X_RTE,U_SHP,U_RTE,Y_SHP,Y_RTE,V_SHP,V_RTE,THETA_SHP,THETA_RTE,BETA_SHP,BETA_RTE,length,traindata,tweet,tweetdata,phi_s1,phi_s2,phi_z)
        #likelyhood=loglikelyhood(X_SHP,X_RTE,U_SHP,U_RTE,Y_SHP,Y_RTE,V_SHP,V_RTE,THETA_SHP,THETA_RTE,BETA_SHP,BETA_RTE,length,traindata,tweet,tweetdata)
        print 'elbo is '+str(elbo)
        #print 'likelyhood is '+str(likelyhood)
        #next for each user,update X , U, Y
        X_SHP = np.ones((1,user),dtype=np.float64)
        X_SHP = X_SHP*random_offset(1,user,shp_off)+hp+topic*hp
        #X_SHP = normalize(X_SHP)
        #X_SHP[1,i] = 
        U_expectation=make_nozero(U_SHP)/make_nozero(U_RTE)
        X_RTE = hp+np.sum(U_expectation,1)
        elbo=ELBO(hp,X_SHP,X_RTE,U_SHP,U_RTE,Y_SHP,Y_RTE,V_SHP,V_RTE,THETA_SHP,THETA_RTE,BETA_SHP,BETA_RTE,length,traindata,tweet,tweetdata,phi_s1,phi_s2,phi_z)
        print 'X_elbo is '+str(elbo)
        #X_RTE = normalize(X_RTE)
        #r_score is the score of the temproal score
        #r_score=predict_score(traindata)
        U_SHP=np.ones((user,topic),dtype=np.float64)*hp
        #U_SHP = normalize(U_SHP)
        for i in range(length):
            #index is match phi_s1 and phi_s2
            #temp_score=r_score[i,3]
            index = traindata[i,0] + traindata[i,1]*user + traindata[i,2]*user*author
            index = int(index)
            #updata U_SHP
            U_SHP[traindata[i,0],:] = U_SHP[traindata[i,0],:] + phi_s1[index,:]+phi_s2[index,:]
        #U_SHP = normalize(U_SHP)
        #make_nozero(U_SHP)
        X_expectation=make_nozero(X_SHP)/make_nozero(X_RTE)
        #U_expectation=make_nozero(U_SHP)/make_nozero(U_RTE)
        ...
        u_part_1 = np.reshape(X_expectation,[user,1])
        u_part_2 = np.sum(THETA_expectation,0)*np.sum(V_expectation,0)
        u_part_3 = np.sum(THETA_expectation,0)*author*(Y_expectation)
        #update U_RTE
        U_RTE = u_part_1+u_part_2+u_part_3
        elbo=ELBO(hp,X_SHP,X_RTE,U_SHP,U_RTE,Y_SHP,Y_RTE,V_SHP,V_RTE,THETA_SHP,THETA_RTE,BETA_SHP,BETA_RTE,length,traindata,tweet,tweetdata,phi_s1,phi_s2,phi_z)
        print 'U_elbo is '+str(elbo)
        #make_nozero(U_RTE)
        #U_RTE = normalize(U_RTE)
        Y_SHP=np.ones((user,topic),dtype=np.float64)*hp
        for i in range(length):
            #temp_score = r_score[i,3]
            index = traindata[i,0] + traindata[i,1]*user + traindata[i,2]*user*author
            index=int(index)
            Y_SHP[traindata[i,0],:] = Y_SHP[traindata[i,0],:]+phi_s2[index,:]
        #Y_SHP=normalize(Y_SHP)
        Y_RTE=np.ones((user,topic),dtype=np.float64)*hp
        Y_RTE=Y_RTE+author*(U_SHP/U_RTE)*np.sum(THETA_SHP/THETA_RTE,0)
        elbo=ELBO(hp,X_SHP,X_RTE,U_SHP,U_RTE,Y_SHP,Y_RTE,V_SHP,V_RTE,THETA_SHP,THETA_RTE,BETA_SHP,BETA_RTE,length,traindata,tweet,tweetdata,phi_s1,phi_s2,phi_z)
        print 'Y_elbo is '+str(elbo)
        #Y_RTE=normalize(Y_RTE)
        #thirdly, for each author,we should upadate V
        V_SHP=np.ones((author,topic),dtype=np.float64)*hp
        for i in range(length):
            temp_score = traindata[i,3]
            index = traindata[i,0] + traindata[i,1]*user + traindata[i,2]*user*author
            index=int(index)
            V_SHP[traindata[i,1],:] = V_SHP[traindata[i,1],:] + phi_s1[index,:]
        #make_nozero(V_SHP)
        #V_SHP = normalize(V_SHP)
        U_expectation=make_nozero(U_SHP)/make_nozero(U_RTE)
        THETA_expectation=make_nozero(THETA_SHP)/make_nozero(THETA_RTE)
        V_RTE = hp*np.ones((author,topic),dtype=np.float64) + np.sum(U_expectation,0)*np.sum(THETA_expectation,0)
        elbo=ELBO(hp,X_SHP,X_RTE,U_SHP,U_RTE,Y_SHP,Y_RTE,V_SHP,V_RTE,THETA_SHP,THETA_RTE,BETA_SHP,BETA_RTE,length,traindata,tweet,tweetdata,phi_s1,phi_s2,phi_z)
        print 'V_elbo is '+str(elbo)
        #make_nozero(V_RTE)
        #V_RTE = normalize(V_RTE)
        #Forth, for each tweet we should upadate THETA
        THETA_SHP=np.ones((tweet,topic),dtype=np.float64)*hp
        for i in range(length):
            #temp_score = traindata[i,3]
            index = traindata[i,0] + traindata[i,1]*user + traindata[i,2]*user*author
            index=int(index)
            THETA_SHP[traindata[i,2],:] = THETA_SHP[traindata[i,2],:]+phi_s1[index,:]+phi_s2[index,:]
        for i in range(tweet):
            for word_index in tweetdata[str(i)]:
                index = int(word_index)+ i*word
                index=int(index)
                THETA_SHP[i,:] = THETA_SHP[i,:]+phi_z[index,:]
        #make_nozero(THETA_SHP)
        #THETA_SHP = normalize(THETA_SHP)
        BETA_expectation=make_nozero(BETA_SHP)/make_nozero(BETA_RTE)
        t_part_1 = np.sum(U_expectation,0)*np.sum(V_expectation,0)
        t_part_2 = author*np.sum(U_expectation*Y_expectation,0)
        t_part_3 = np.sum(BETA_expectation,0)
        THETA_RTE = hp*np.ones((tweet,topic),dtype=np.float64)+t_part_1+t_part_2+t_part_3
        elbo=ELBO(hp,X_SHP,X_RTE,U_SHP,U_RTE,Y_SHP,Y_RTE,V_SHP,V_RTE,THETA_SHP,THETA_RTE,BETA_SHP,BETA_RTE,length,traindata,tweet,tweetdata,phi_s1,phi_s2,phi_z)
        print 'TH_elbo is '+str(elbo)
        #pdb.set_trace() 
        #make_nozero(THETA_RTE)
        #THETA_RTE=normalize(THETA_RTE)
        #finally,for each word we should update beta
        BETA_SHP=np.ones((word,topic),dtype=np.float64)*hp
        for i in range(tweet):
            for word_index in tweetdata[str(i)]:
                index = int(word_index)+i*word
                index=int(index)
                BETA_SHP[int(word_index),:] = BETA_SHP[int(word_index),:]+phi_z[index,:]
        #make_nozero(BETA_SHP)
        #BETA_SHP = normalize(BETA_SHP)
        THETA_expectation=make_nozero(THETA_SHP)/make_nozero(THETA_RTE)
        BETA_RTE = hp*np.ones((word,topic),dtype=np.float64)+np.sum(THETA_expectation,0)
        elbo=ELBO(hp,X_SHP,X_RTE,U_SHP,U_RTE,Y_SHP,Y_RTE,V_SHP,V_RTE,THETA_SHP,THETA_RTE,BETA_SHP,BETA_RTE,length,traindata,tweet,tweetdata,phi_s1,phi_s2,phi_z)
        print 'BE_elbo is '+str(elbo)
        print 'flag'
        #make_nozero(BETA_RTE)
        #BETA_RTE=normalize(BETA_RTE)
        ##################################################################
        #now we should keep the conditional to break    
        elbo=ELBO(hp,X_SHP,X_RTE,U_SHP,U_RTE,Y_SHP,Y_RTE,V_SHP,V_RTE,THETA_SHP,THETA_RTE,BETA_SHP,BETA_RTE,length,traindata,tweet,tweetdata,phi_s1,phi_s2,phi_z)
        #likelyhood=loglikelyhood(X_SHP,X_RTE,U_SHP,U_RTE,Y_SHP,Y_RTE,V_SHP,V_RTE,THETA_SHP,THETA_RTE,BETA_SHP,BETA_RTE,length,traindata,tweet,tweetdata)
        print 'elbo is '+str(elbo)
        #print 'likelyhood is '+str(likelyhood)
