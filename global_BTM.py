...
def update_phi_s(user_index,author_index,tweet_index,U_SHP_D,U_RTE_L,V_SHP_D,V_RTE_L,Y_SHP_D,Y_RTE_L,THETA_SHP_D,THETA_RTE_L):
    ...
    temp = U_SHP_D[user_index,:] - U_RTE_L[user_index,:] + THETA_SHP_D[tweet_index,:] - THETA_RTE_L[tweet_index,:]
    part_s1 = V_SHP_D[author_index,:] - V_RTE_L[author_index,:]
    part_s2 = Y_SHP_D[0,:]- Y_RTE_L[0,:]
...
def update_phi_z(word_index,tweet_index,THETA_SHP_D,THETA_RTE_L,BETA_SHP_D,BETA_RTE_L):
    res = THETA_SHP_D[tweet_index,:] - THETA_RTE_L[tweet_index,:] + BETA_SHP_D[word_index,:] - BETA_RTE_L[word_index,:]
...
def  ELBO(hp,X_SHP,X_RTE,U_SHP,U_RTE,Y_SHP,Y_RTE,V_SHP,V_RTE,THETA_SHP,THETA_RTE,BETA_SHP,BETA_RTE,THETA_RTE_HP,BETA_RTE_HP):
    ...
    term5=np.sum((hp-THETA_SHP)*(digamma(THETA_SHP)-np.log(THETA_RTE))-(THETA_RTE_HP-THETA_RTE)*THETA_expectation+gammaln(THETA_SHP)-THETA_SHP*np.log(THETA_RTE))
    BETA_expectation=make_nozero(BETA_SHP)/make_nozero(BETA_RTE)
    #make_nozero(BETA_expectation)
    term6=np.sum((hp-BETA_SHP)*(digamma(BETA_SHP)-np.log(BETA_RTE))-(BETA_RTE_HP-BETA_RTE)*BETA_expectation+gammaln(BETA_SHP)-BETA_SHP*np.log(BETA_RTE))
...
def BTM_initialize(THETA_SHP,BETA_SHP):
    THETA_expectation = np.loadtxt('k30.txt')
    print THETA_SHP.shape
    print THETA_expectation.shape
    THETA_RTE=make_nozero(THETA_SHP/THETA_expectation)
    BETA_expectation = np.loadtxt('word_probabaly.txt')
...
def process(user,tweet,word,author,topic,hp,shp_off,rte_off,BTM,length):
    ...
    Y_SHP = np.ones((1,topic),dtype=np.float64)
    Y_RTE = np.ones((1,topic),dtype=np.float64)
    Y_SHP = Y_SHP*random_offset(1,topic,shp_off)+hp
    Y_SHP = Y_SHP*random_offset(1,topic,rte_off)+hp
    ...
    if BTM:
        THETA_RTE,BETA_RTE=BTM_initialize(THETA_SHP,BETA_SHP) 
        THETA_RTE_HP=THETA_RTE
        BETA_RTE_HP = BETA_RTE
    ...
    while 1:
        ...
        #save every digamma fuanciton result
        X_SHP_D=digamma(X_SHP)
        U_SHP_D=digamma(U_SHP)
        V_SHP_D=digamma(V_SHP)
        Y_SHP_D=digamma(Y_SHP)
        THETA_SHP_D=digamma(THETA_SHP)
        BETA_SHP_D=digamma(BETA_SHP)
        #save some log function result
        X_RTE_L=np.log(X_RTE)
        U_RTE_L=np.log(U_RTE)
        V_RTE_L=np.log(V_RTE)
        Y_RTE_L=np.log(Y_RTE)
        THETA_RTE_L=np.log(THETA_RTE)
        BETA_RTE_L=np.log(BETA_RTE)
        ...
        Y_SHP=np.ones((1,topic),dtype=np.float64)*hp
        V_SHP=np.ones((author,topic),dtype=np.float64)*hp
        THETA_SHP=np.ones((tweet,topic),dtype=np.float64)*hp
        for i in range(length):
            phi_s1,phi_s2 = update_phi_s(traindata[i,0],traindata[i,1],traindata[i,2],U_SHP_D,U_RTE_L,V_SHP_D,V_RTE_L,Y_SHP_D,Y_RTE_L,THETA_SHP_D,THETA_RTE_L)
        ...
        Y_RTE=np.ones((1,topic),dtype=np.float64)*hp
        Y_RTE=Y_RTE+author*np.sum((make_nozero(U_SHP)/make_nozero(U_RTE)),0)*np.sum(make_nozero(THETA_SHP_TEMP)/make_nozero(THETA_RTE_TEMP),0)
        ...
        for i in range(tweet):
            for j in tweetdata[str(i)]:
                phi_z = update_phi_z(int(j),i,THETA_SHP_D,THETA_RTE_L,BETA_SHP_D,BETA_RTE_L)
        ...
        t_part_2 = author*np.sum(U_SHP/U_RTE,0)*(Y_SHP/Y_RTE)
        t_part_3 = np.sum(BETA_SHP_TEMP/BETA_RTE_TEMP,0)
        THETA_RTE = THETA_RTE_HP +t_part_1+t_part_2+t_part_3
        BETA_RTE = BETA_RTE_HP+np.sum(THETA_SHP/ THETA_RTE,0)
        ...
        if iter%1==0:
            ...
            for i in range(length):
                ...
                temp=phi_s1*(digamma(U_SHP[traindata[i,0],:])-np.log(U_RTE[traindata[i,0],:])+digamma(V_SHP[traindata[i,1],:])-np.log(V_RTE[traindata[i,1],:])+digamma(THETA_SHP[traindata[i,2],:])-np.log(THETA_RTE[traindata[i,2],:]))+\
                        phi_s2*(digamma(U_SHP[traindata[i,0],:])-np.log(U_RTE[traindata[i,0],:])+digamma(THETA_SHP[traindata[i,2],:])-np.log(THETA_RTE[traindata[i,2],:])+digamma(Y_SHP)-np.log(Y_RTE))\
                            -phi_s1*np.log(phi_s1)-phi_s2*np.log(phi_s2)
                term7=term7+np.sum(temp)
            term7=term7-np.sum(np.sum(THETA_expectation,0)*np.sum(V_expectation,0)*np.sum(U_expectation,0))-author*np.sum(np.sum(THETA_expectation,0)*np.sum(U_expectation,0)*Y_expectation)
            term8=0
            for i in range(tweet):
                for j in tweetdata[str(i)]:
                    phi_z = update_phi_z(int(j),i,THETA_SHP_D,THETA_RTE_L,BETA_SHP_D,BETA_RTE_L)
            ...
            elbo=term7+term8+(user+ user* topic+ author * topic) * -1.4570 + (user * topic+tweet*topic+author*topic)*-1.0957+np.sum(0.3*np.log(BETA_RTE_HP))+np.sum(0.3*np.log(THETA_RTE_HP))

            elbo=elbo+ELBO(hp,X_SHP,X_RTE,U_SHP,U_RTE,Y_SHP,Y_RTE,V_SHP,V_RTE,THETA_SHP,THETA_RTE,BETA_SHP,BETA_RTE,THETA_RTE_HP,BETA_RTE_HP)
            print 'iter is  '+str(iter)
            print 'elbo is  '+str(elbo)
